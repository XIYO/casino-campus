...
@Tag(name = "JPA 회원 관리", description = "JPA Repository를 사용한 회원 관리 API")
@RestController
...
public class JpaMemberController {
    ...
    public JpaMemberController(@Qualifier("jpaMemberService") IMemberService memberService) {
    ...
    public ResponseEntity<MemberDto.Response> createMember(
        ...
        log.info("JPA 회원가입 요청: {}", request.getEmail());
        
        try {
            MemberDto.Response response = memberService.createMember(request);
            return ResponseEntity.ok(response);
        } catch (IllegalArgumentException e) {
            log.error("JPA 회원가입 실패: {}", e.getMessage());
            return ResponseEntity.badRequest().build();
        } catch (UnsupportedOperationException e) {
            log.error("JPA 구현이 필요합니다: {}", e.getMessage());
    ...
    @Operation(summary = "ID로 회원 조회", description = "회원 ID로 특정 회원을 조회합니다")
    @ApiResponse(responseCode = "200", description = "조회 성공",
                content = @Content(schema = @Schema(implementation = MemberDto.Response.class)))
    ...
    public ResponseEntity<MemberDto.Response> getMemberById(
        ...
        log.info("JPA 회원 조회 요청: ID {}", id);
        
        try {
            MemberDto.Response response = memberService.getMemberById(id);
            return ResponseEntity.ok(response);
        } catch (IllegalArgumentException e) {
            log.error("JPA 회원 조회 실패: {}", e.getMessage());
    ...
    @Operation(summary = "이메일로 회원 조회", description = "이메일 주소로 특정 회원을 조회합니다")
    @ApiResponse(responseCode = "200", description = "조회 성공",
                content = @Content(schema = @Schema(implementation = MemberDto.Response.class)))
    @ApiResponse(responseCode = "404", description = "회원을 찾을 수 없음")
    @ApiResponse(responseCode = "501", description = "구현 필요")
    ...
    public ResponseEntity<MemberDto.Response> getMemberByEmail(
        ...
        log.info("JPA 회원 조회 요청: Email {}", email);
    ...
    public ResponseEntity<Boolean> checkEmailExists(
        ...
        log.info("JPA 이메일 중복 확인 요청: {}", email);
